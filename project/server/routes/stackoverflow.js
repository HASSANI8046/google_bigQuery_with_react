...
function queryStackOverflow(projectId, query_id, game_id) {
  ...
  var sqlQuery
  if (query_id === 'AI') {
    sqlQuery = `SELECT * FROM \`jupiter-2248.games_dashboard.AI\` where game=@game order by Install_time desc`;
  }
  if (query_id === 'A') {
    sqlQuery = `SELECT * FROM \`jupiter-2248.games_dashboard.A\` where game=@game order by Install_time desc`;
  }
  ...
  if (query_id === 'I') {
    sqlQuery = `SELECT * FROM \`jupiter-2248.games_dashboard.I\` where game=@game order by Install_time desc`;
  }
  ...
  if (query_id === 'AIMA') {
    sqlQuery = `SELECT * FROM \`jupiter-2248.games_dashboard.MAI\` where game=@game`;
  }
